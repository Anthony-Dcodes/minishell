...
/*   Updated: 2026/06/03 15:11:43 by oem5491          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	isolate_and_replace_env(t_s *s, t_token *t, int *index, char **envp)
{
	int		end_index;
	int		start_index;
	char	*substring;
	...
	start_index = *index + 1;
	end_index = get_variable_end_index(t->value, start_index + 1);
	if (get_string(start_index, end_index, t->value, &substring) == ERR_MALLOC)
		return (ERR_VAR_SUBST);
	if (ft_equal2(s, substring, "?", TRUE))
		ret = ft_getenv2d(s, envp, L_SHLES, &env_var);
	else
		ret = ft_wgetenv2d(s, envp, substring, &env_var);
	if (ret < ERROR)
		return (ERR_VAR_SUBST);
	free(substring);
	if (replace_variable(t, env_var, *index, end_index) != ERR_OK)
		return (ERR_VAR_SUBST);
	if (env_var != NULL)
